...
import (
	"context"
	"github.com/wave2588/go-base/cache/store"
...
var _ MemCacher = (*MemCache)(nil)

func NewMemCache(store store.Store, expire time.Duration, opts ...Option) *MemCache {
	r := &MemCache{
		store:  store,
		expire: expire,
	}
...
func (c *MemCache) canFallbackWhenError() bool {
...
func (c *MemCache) get(ctx context.Context, keyFunc KeyFunc, fallbackFunc FallbackFunc, dst interface{}, expire time.Duration) error {
	...
	if err == nil {
		...
		left := utils.RecursiveIndirectType(dstV.Type())
		right := utils.RecursiveIndirectType(fV.Type())
...
func (c *MemCache) Get(ctx context.Context, keyFunc KeyFunc, fallbackFunc FallbackFunc, dst interface{}, ttl *time.Duration) error {
	expire := c.expire
	if ttl != nil {
		expire = *ttl
	}
...
func (c *MemCache) MustGet(ctx context.Context, keyFunc KeyFunc, fallbackFunc FallbackFunc, dst interface{}, ttl *time.Duration) {
	utils.PanicIf(c.Get(ctx, keyFunc, fallbackFunc, dst, ttl))
}

func (c *MemCache) getMulti(ctx context.Context, ids interface{},
	...
	dstType := utils.RecursiveIndirectType(reflect.TypeOf(dstMap))
	...
	left = utils.RecursiveIndirectType(utils.RecursiveIndirect(reflect.ValueOf(dstMap)).Type().Elem())
...
func (c *MemCache) MustGetMulti(ctx context.Context, ids interface{},
	keyFunc KeyMultiFunc,
	fallbackFunc FallbackMultiFunc,
	dstMap interface{},
	ttl *time.Duration) {
	utils.PanicIf(c.GetMulti(ctx, ids, keyFunc, fallbackFunc, dstMap, ttl))
}

func (c *MemCache) Evict(ctx context.Context, keyFunc KeyFunc) error {
	return c.store.Delete(ctx, keyFunc())
}

func (c *MemCache) MustEvict(ctx context.Context, keyFunc KeyFunc) {
	utils.PanicIf(c.Evict(ctx, keyFunc))
}

func (c *MemCache) EvictMulti(ctx context.Context, ids interface{}, keyFunc KeyMultiFunc) error {
	// check slice type
...
func (c *MemCache) MustEvictMulti(ctx context.Context, ids interface{}, keyFunc KeyMultiFunc) {
	utils.PanicIf(c.EvictMulti(ctx, ids, keyFunc))
}

func (c *MemCache) refresh(ctx context.Context, keyFunc KeyFunc, fallbackFunc FallbackFunc, p reflect.Type, expire time.Duration) error {
	key := keyFunc()
...
func (c *MemCache) Refresh(ctx context.Context, keyFunc KeyFunc, fallbackFunc FallbackFunc, i interface{}, ttl *time.Duration) error {
	expire := c.expire
	if ttl != nil {
		expire = *ttl
	}
...
func (c *MemCache) MustRefresh(ctx context.Context, keyFunc KeyFunc, fallbackFunc FallbackFunc, i interface{}, ttl *time.Duration) {
	utils.PanicIf(c.Refresh(ctx, keyFunc, fallbackFunc, i, ttl))
}

func (c *MemCache) refreshMulti(ctx context.Context, ids interface{}, keyFunc KeyMultiFunc,
...
func (c *MemCache) MustRefreshMulti(ctx context.Context, ids interface{}, keyFunc KeyMultiFunc,
	fallbackFunc FallbackMultiFunc, dst interface{}, ttl *time.Duration) {
	expire := c.expire
	if ttl != nil {
		expire = *ttl
	}
	utils.PanicIf(c.refreshMulti(ctx, ids, keyFunc, fallbackFunc, dst, expire))
}
